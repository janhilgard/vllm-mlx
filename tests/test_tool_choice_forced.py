...
class TestApplyForcedToolChoice:
    ...
    def test_forced_unknown_tool_keeps_all(self):
        """If forced tool name doesn't match any tool, keep all tools."""
        tools = [WEATHER_TOOL, CALC_TOOL]
        msgs = self._make_messages()
        tc = {"type": "function", "function": {"name": "nonexistent"}}

        new_tools, _ = _apply_forced_tool_choice(tc, tools, msgs)

        # No match found, all tools kept
